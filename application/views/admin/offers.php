...
<head>
	...
	<body id="kt_body" class="header-fixed header-mobile-fixed subheader-enabled subheader-fixed aside-enabled aside-fixed aside-minimize-hoverable page-loading">
							...
							<div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
								...
								<div class="d-flex align-items-center flex-wrap mr-1">
									...
									<div class="d-flex align-items-baseline flex-wrap mr-5">
										<!--begin::Page Title-->
										<h5 class="text-dark font-weight-bold my-1 mr-5">Offers List</h5>
										<!--end::Page Title-->
										<!--begin::Breadcrumb-->
										<ul class="breadcrumb breadcrumb-transparent breadcrumb-dot font-weight-bold p-0 my-2 font-size-sm">
											...
											<li class="breadcrumb-item">
												<a href="#" class="text-muted">Offers List</a>
											</li>
										</ul>
										<!--end::Breadcrumb-->
									</div>
								...
								<div class="card card-custom card-stretch gutter-b">
											...
											<div class="card-header border-0 py-5">
												<h3 class="card-title align-items-start flex-column">
													...
													<span class="text-muted mt-3 font-weight-bold font-size-sm">More than 10+ new offers active</span>
												</h3>
											...
											<div class="card-body pt-0 pb-3">
												<div class="tab-content">
													...
													<div class="table-responsive">
														<table class="table table-head-custom table-head-bg table-borderless table-vertical-center" id="example">
															<thead>
																<tr class="text-left text-uppercase">
																	...
																	<th style="min-width: 70px">Percentage / Amount</th>
																	<th style="min-width: 70px">Start date</th>
																	<th style="min-width: 70px">End date</th>
																	<th style="min-width: 70px">Status</th>
																	<th style="min-width: 70px">Action</th>
																</tr>
															</thead>
															<tbody>
														<?php if($offers){ foreach($offers as $value){  ?>	    
																<tr>
																	<td class="pl-0 py-8">
																		<div class="d-flex align-items-center">
																			<div class="symbol symbol-100 symbol-light mr-1">
																				
																					<img src="<?=base_url()?>assets/media/users/<?=$value->banner?>" class="h-800 align-self-center" alt="">
																	...
																	<td>
																		<span class="text-dark-75 font-weight-bolder d-block font-size-lg"><?=$value->code?></span>
																		
																	</td>
																	<td>
																	    <?php  if($value->type=="amount"){  ?>
																		<span class="text-dark-75 font-weight-bolder d-block font-size-lg"><?=$value->amount?></span>
																		<span class="text-muted font-weight-bold"><?=$value->type?></span>
																		
																		<?php }else { ?>
																		<span class="text-dark-75 font-weight-bolder d-block font-size-lg"><?=$value->percentage?>%</span>
																		<span class="text-muted font-weight-bold"><?=$value->type?></span>
																		<?php } ?>
																	</td>
																	<td>
																		<span class="text-dark-75 font-weight-bolder d-block font-size-lg"><?=$value->start_date?></span>
																		
																	</td>
																	<td>
																		<span class="text-dark-75 font-weight-bolder d-block font-size-lg"><?=$value->end_date?></span>
													  ...
													  <?php if($value->status=="0"){ ?>
													     <a href="<?=base_url('Admin/deactiveoffers')?>/<?=$value->id?>"><span class="label label-lg font-weight-bold label-light-danger label-inline">Dactive </span></a>
													    <?php  }else{ ?>
													      <a href="<?=base_url('Admin/activeoffers')?>/<?=$value->id?>"><span class="label label-lg font-weight-bold label-light-success label-inline">Active </span></a>
													      <?php } ?>
													 </td>
																	<td class="pr-0 ">
																		<a href="<?=base_url('Admin/offerupdate/'.$value->id)?>" class="btn btn-light-success font-weight-bold mr-2"><i class="la la-edit"></i></a>
																		<a href="<?=base_url('Admin/offerdelete/'.$value->id)?>" onclick="return confirm('Are you sure want to delete ?');"  href="#" class="btn btn-light-danger font-weight-bold mr-2"><i class="la la-trash"></i> </a>
																	</td>
																</tr>
																<?php } }  ?>
      ...
      <script type="text/javascript">
		...
		<script>
		    function banner (id,status){
		        if(status=="0"){
		            $('#btns'+id).html('Deactivate')
		        }else{
		             $('#btns'+id).html('Activate')
		        }
		        
		         $.ajax({
		               url:"<?=base_url()?>Admin/offerstatus",
		               method:"POST",
